...
class Trainer(object):
    def __init__(self, args, noter):
        if args.no_ctx:
            from dataloaders.dataloader_no_ctx import get_dataloader
            from models.CPMR_no_ctx import CPMR
        else:
            from dataloaders.dataloader_no_his import get_dataloader
            from models.CPMR_no_his import CPMR
    ...
    def run_train(self):
        loss_tbptt, loss_epoch, count_tbptt, count_total = 0., 0., 0, 0
        ...
        for batch in tqdm(self.trainloader, desc='  - training', leave=False):
            loss_batch, _, _, self.xu_t_plus, self.xi_t_plus = self.model(batch, self.xu_t_plus, self.xi_t_plus)

            loss_tbptt += loss_batch
            loss_epoch += loss_batch.item() / self.len_train_dl
            count_tbptt += 1
            count_total += 1
        ...
        self.noter.log_train(loss_epoch, time.time() - time_start)
    ...
    def rollout(self, mode: str):
        ...
        with torch.no_grad():
            for batch in tqdm(dl, desc='  - ' + mode, leave=False):
                ...
                loss_batch, zu_pos, zi, self.xu_t_plus, self.xi_t_plus = \
