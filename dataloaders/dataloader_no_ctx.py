...
class Dataset:
    ...
    def get_item(self, idx):
        ...
        tgt_u_neg = \
            np.array([self.all_u[~self.all_u.isin(df_his_tgt[df_his_tgt['item'] == i]['user'])].sample(
                self.n_neg_sample).values
                      for i in tgt_i])
        tgt_i_neg = \
            np.array([self.all_i[~self.all_i.isin(df_his_tgt[df_his_tgt['user'] == u]['item'])].sample(
                self.n_neg_sample).values
                      for u in tgt_u])
        # assert (self.idx_pad not in tgt_u_neg) and (self.idx_pad not in tgt_i_neg) and (self.idx_pad not in tgt_i)

        df_his = self.df.iloc[: idx_tgt_start]
        if idx == 0:
            adj_his = self.get_adj_ui(df_his.iloc[: idx_tgt_start])
        else:
            adj_his = self.get_adj_ui(df_his.iloc[self.cum_n_records[idx - 1]: idx_tgt_start])

        return ts_inc, adj_his, None, adj_tgt, tgt_u, tgt_i, tgt_u_neg, tgt_i_neg
...
class Dataloader:
    ...
    def _get_iter(self, start_idx=0):
        adj_his, adj_his_inc = None, None

        for i in range(start_idx, len(self.ds)):
            ts_inc, adj_his_inc, _, adj_tgt, tgt_u, tgt_i, tgt_u_neg, tgt_i_neg = self.ds.get_item(i)

            adj_his = adj_his_inc if adj_his is None else adj_his + adj_his_inc
            adj_his_laplacian = sparse_mx_to_torch_sparse_tensor(bi_adj_to_laplacian(adj_his) * self.alpha_spectrum
                                                                 ).to(self.device)
            ...
            yield \
                ts_inc, adj_his_laplacian, None, adj_tgt_i2u, adj_tgt_u2i, tgt_u, tgt_i, tgt_u_neg, \
                tgt_i_neg
