...
class CPMR(nn.Module):
    ...
    def forward(self, batch, xu_in, xi_in):
        t_diff, _, adj_ctx, adj_tgt_i2u, adj_tgt_u2i, tgt_u, tgt_i, tgt_u_neg, tgt_i_neg = batch

        # propagate during non-events
        xu_t_minus, xi_t_minus = \
            self.evolver(adj_ctx, t_diff, xu_in, xi_in, self.embeds_u.weight, self.embeds_i.weight)
